...
```{r warning=FALSE}
ppdata <- read.xlsx("MillerPainTreatmentCenterData.xlsx", 4, header=TRUE)
...
hist(mydata$Teach, main="Histogram of observed data",
     xlab = "Teach", prob = TRUE, ylim=c(0, 0.10)
     , col="peachpuff")
...
legend(20, 0.07, legend=c("Exponential", "Triangular" , "Gamma"), 
...
hist(mydata$Resident.Attending, main="Histogram of observed data",
     xlab = "Resident and attending", prob = TRUE, ylim=c(0, 0.1)
     , col="peachpuff")
...
RandomTriang = rtri(nrow(mydata),
         min = min(mydata$Resident.Attending),
         max = max(mydata$Resident.Attending),
         mode = Mode(mydata$Resident.Attending)[1])
...
legend(30, 0.08, legend=c("Weibull", "Triangular" ,"Gamma"),
       fill = c("red","blue","black"), bty="n")

ks.test(mydata$Resident.Attending, RandomWeibull)

fitweibull
ks.test(mydata$Resident.Attending, RandomTriang)
